...
def sameP2P(addr1, addr2):
    # Returns true if the two addresses provided are one apart (assume same
    # /30 or /31 network)
...
def Reduce(list):
    # Takes a list of links and merges entries for two ends of the same link,
    # provided the metric matches
    l = 0
    while(l < len(list) - 1):
        if(list[l][0] == list[l + 1][1] and list[l + 1][0] == list[l][1] and sameP2P(list[l][2], list[l + 1][2]) and list[l][3] == list[l + 1][3]):
            # if two links are A->B and B->A and in same subnet and have same
            # metric then merge into an undirected edge
            list[l][4] = 'none'
...
def mergeSort(list):
    # Performs a standard merge sort on a list of link entries based on IP
    # address
    sorted = []
    listlen = len(list)
    i = 0
...
with open(filename, 'r') as infile:
    for line in infile:
        ...
        if(m):
            ...
            continue
        m = re.search(
            '\(Link ID\) Network/subnet number: (\d*.\d*.\d*.\d*)', line)
        if(m):
            stubnet = m.group(1)
            continue
        ...
        if(m):
            ...
            continue
        m = re.search(
            '\(Link ID\) Neighboring Router ID: (\d*.\d*.\d*.\d*)', line)
        if(m):
            neighbour = m.group(1)
            continue
        m = re.search(
            '\(Link Data\) Router Interface address: (\d*.\d*.\d*.\d*)', line)
        if(m):
            interfaceip = m.group(1)
            continue
        m = re.search(
            '\(Link ID\) Designated Router address: (\d*.\d*.\d*.\d*)', line)
        if(m):
            transit = m.group(1)
            continue
...
with open(filename, 'w') as outfile:
    ...
    for t in transits:
        ...
        outfile.write('\tt' + re.sub('\.', 'x', t) +
                      ' [label="LAN with DR\\n' + t + '", shape=box]\n')
    for r in routers:
        for t in r.transits:
            # Dump transit connections
            outfile.write('\th' + re.sub('\.', 'x', r.routerid) +
                          ' -> t' + re.sub('\.', 'x', t[0]) + '[label="' + t[1] + '"]\n')
        for l in r.links:
    ...
    for l in Reduce(mergeSort(links)):
        ...
        outfile.write('\th' + re.sub('\.', 'x', l[0]) + ' -> h' + re.sub(
            '\.', 'x', l[1]) + '[label="' + l[3] + '", dir=' + l[4] + ']\n')
    outfile.write("}\n")
