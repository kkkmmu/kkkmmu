...
func IsSubDirExist(root string, sdir string) bool {
	var exist bool = false
	err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		if info.Name() == sdir && info.IsDir() {
			exist = true
		}
		return nil
	})
...
func IsSubPathExist(root string, path string) bool {
	var exist bool = false
	err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		if info.Name() == path {
			exist = true
...
func GetAbsOfSubDir(root string, sdir string) (string, error) {
	var exist bool = false
	var abs string
	err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		if info.Name() == sdir && info.IsDir() {
	...
	if !exist {
		return "", fmt.Errorf("there is no %s under %s", sdir, root)
	}

	return abs, nil
}

func GetAbsOfSubPath(root string, path string) (string, error) {
	var exist bool = false
	var abs string
	err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		if info.Name() == path {
	...
	if !exist {
		return "", fmt.Errorf("there is no %s under %s", path, root)
	}
