...
/*VTY SHELL host name. */
var hostRegPp = regexp.MustCompile("^(?P<host>.*)>")

/* Real host name. */
var hostRegPr = regexp.MustCompile(`^(?P<host>.*)\[`)
...
func (c *Cli) GetHostname() (string, string, error) {
	...
	var pHostname string
	var rHostname string
	lines := strings.Split(string(data), "\n")
	for _, line := range lines {
		matches := hostRegPp.FindStringSubmatch(line)
		if len(matches) == 0 {
			continue
		}
		pHostname = matches[1]
	...
	for _, line := range lines {
		matches := hostRegPr.FindStringSubmatch(line)
		if len(matches) == 0 {
			continue
		}
		rHostname = matches[1]
	...
	pHostname = strings.TrimSpace(pHostname)
	rHostname = strings.TrimSpace(rHostname)
	for {
		if pHostname == "" || rHostname == "" {
	...
	pHostname = strings.Trim(pHostname, "*")
	if rHostname != "" {
		return pHostname, rHostname, nil
	}
