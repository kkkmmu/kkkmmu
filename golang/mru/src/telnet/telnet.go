...
func newSession(addr string) (*Session, error) {
	...
	c := Session{
		Conn: conn,
		r:    bufio.NewReaderSize(conn, 10000000),
	}
	return &c, nil
}
...
func (s *Session) ReadAll() ([]byte, int, error) {
	var n int
...
func (s *Session) ReadUntilContains(delims, cdelims []string) ([]byte, error) {
	if len(cdelims) > 0 {
		result := make([]byte, 0, 64)
		for {
			line, err := s.ReadUntil(delims...)
			...
			result = append(result, line...)
			for _, ed := range cdelims {
				fmt.Println(string(line), ed)
				if strings.Contains(string(line), ed) {
					goto RETURN
				}
			}
...
func (s *Session) ReadTimeout(timeout time.Duration) ([]byte, error) {
	buf := make([]byte, 0, 100000)
	bch := make(chan byte, 10000)
	ech := make(chan error, 1)

	go func(bch chan byte, ech chan error) {
		b, retry, err := s.tryReadByte()
		if err != nil {
			ech <- err
		}
...
func New(user, pass, ip, port string) (*Session, error) {
	...
	s.WriteLine(pass)
	data, err := s.ReadUntil([]string{">", "#", "incorrect"}...)
	if err != nil {
		fmt.Println("Error happend when login: ", err.Error())
		return nil, err
	}

	if strings.Contains(string(data), "incorrect") {
		_, err = s.ReadUntil("ogin:")
		return nil, errors.New("Invalid password: " + pass)
	}
