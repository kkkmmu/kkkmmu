...
func (ap *AddressPool) SetSutIPAddress(ip string) error {
	...
	err = ap.EnableTrafficDestination()
	if err != nil {
		return fmt.Errorf("Cannot set sut as traffic destination with %s", err)
	}
	return nil
}
...
func (ap *AddressPool) SetTesterAddresses(vid, ip, plen, mac, count string) error {
	err := ap.SetTesterIPAddresses(ip, plen, count)
	if err != nil {
		return err
	}
	...
	return ap.SetTesterMacAddresses(mac, count)
}
...
func (ap *AddressPool) SetTesterAddresses6(vid, ip, plen, mac, count string) error {
	err := ap.SetTesterIP6Addresses(ip, plen, count)
	if err != nil {
		return err
	}
	...
	return ap.SetTesterMacAddresses6(mac, count)
}
...
func (ap *AddressPool) SetTesterIPAddresses(ip, plen, count string) error {
	cmd := fmt.Sprintf("AgtEthernetAddressPool SetTesterIpAddresses %s %s %s %s %s", ap.Handler, ip, plen, count, "1")
	_, err := ap.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot set tester ip address for pool %s port: %s : %s", ap.Handler, ap.Port.Name, err.Error())
	}
...
func (ap *AddressPool) SetTesterIP6Addresses(ip, plen, count string) error {
	cmd := fmt.Sprintf("AgtEthernetIpv6AddressPool SetTesterAddresses %s %s %s %s %s 0", ap.Handler, ip, plen, count, "::1")
	_, err := ap.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot set tester ip6 address for pool %s port: %s : %s", ap.Handler, ap.Port.Name, err.Error())
	}
...
func (ap *AddressPool) SetTesterMacAddresses(mac, count string) error {
	cmd := fmt.Sprintf("AgtEthernetAddressPool SetTesterMacAddresses %s %s 1", ap.Handler, strings.TrimSpace(mac))
	_, err := ap.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot set tester mac address for pool %s port: %s : %s", ap.Handler, ap.Port.Name, err.Error())
...
func (ap *AddressPool) SetTesterMacAddresses6(mac, count string) error {
	cmd := fmt.Sprintf("AgtEthernetIpv6AddressPool SetTesterMacAddresses %s %s 1", ap.Handler, strings.TrimSpace(mac))
	_, err := ap.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot set tester mac address for ipv6 pool %s port: %s : %s", ap.Handler, ap.Port.Name, err.Error())
