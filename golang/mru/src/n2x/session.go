...
func (ns *NSession) ReservePort(name string) (*Port, error) {
	if ns.Ports == nil {
		ns.Ports = make(map[string]*Port, 10)
	}

	if !ns.isPortValid(name) {
		return nil, fmt.Errorf("port %s is invalid on n2x", name)
	}

	fields := strings.Split(name, "/")

	var handle string
	wait := time.Tick(time.Second * 10)
	for {
		cmd := fmt.Sprintf("AgtPortSelector AddPort %s %s", fields[0], fields[1])
		res, err := ns.Invoke(cmd)
		if err != nil {
			if strings.Contains(err.Error(), "It may be powered off or in the process of rebooting") {
				<-wait
				continue
			}
			return nil, err
...
func (ns *NSession) ReleasePort(name string) error {
	if !ns.isPortValid(name) {
		return fmt.Errorf("port %s is invalid on n2x", name)
	}

	np, ok := ns.Ports[name]
	if !ok {
		return fmt.Errorf("port %s is not reserved", name)
	}

	cmd := fmt.Sprintf("AgtPortSelector RemovePort %s", np.Handler)
	_, err := ns.Invoke(cmd)
	if err != nil {
		return err
	}

	delete(ns.Ports, name)
...
func (ns *NSession) Lock() {
	ns.lock.Lock()
}

func (ns *NSession) Unlock() {
	ns.lock.Unlock()
}
...
func (ns *NSession) Reset() error {
	cmd := fmt.Sprintf("AgtTestTopology ResetSession %s", ns.Port)
	_, err := ns.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot reset sesssion %s with %s", ns.Label, err.Error())
	}
