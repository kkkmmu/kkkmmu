...
func (p *Port) Init() error {
	_, err := p.GetAvailableMediaTypes()
	if err != nil {
		return fmt.Errorf("Get port media type error with %s", err)
	}

	_, err = p.LegacyLinkGetAllAddressPools()
	if err != nil {
		return fmt.Errorf("Get port legacy link address pool failed with %s", err)
	}

	_, err = p.LegacyLinkGetAllIP6AddressPools()
	if err != nil {
		return fmt.Errorf("Get port legacy link ipv6 address pool failed with %s", err)
	}

	err = p.InitStatistics()
	if err != nil {
		return fmt.Errorf("Init port %s default statisticsfailed with %s", p.Name, err)
	}

	return nil
}

func (p *Port) StatisticsName() string {
	return fmt.Sprintf("ATSN2X%s", strings.Replace(p.Name, "/", "_", -1))
}

func (p *Port) IsStatisticsConfigured() bool {
	cmd := fmt.Sprintf("AgtStatisticsList ListNames")
	...
	if strings.Contains(res, p.StatisticsName()) {
		return true
	}
...
func (p *Port) SyncStatistics() (*Statistics, error) {
	st := &Statistics{
		Name: p.StatisticsName(),
		Port: p,
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Cannot get statistics name with: %s", err)
	}
...
func (p *Port) RemoveStatistics(st *Statistics) error {
	handle, err := st.GetHandle()
	if err != nil {
		return fmt.Errorf("Cannot remote statistics with: %s", err)
	}

	cmd := fmt.Sprintf("AgtStatisticsList Remove %s", handle)
	_, err = p.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot remove statistics %s with %s", st.Name, err)
	}

	return nil
}

func (p *Port) AddStatistics() (*Statistics, error) {
	p.RemoveStatistics(&Statistics{Name: p.StatisticsName(), Port: p})

	cmd := fmt.Sprintf("AgtStatisticsList Add AGT_STATISTICS")
	res, err := p.Invoke(cmd)
	if err != nil {
		return nil, fmt.Errorf("Cannot add statistics with %s", err)
	}
	...
	if res == "" {
		return nil, fmt.Errorf("Cannot add statistics %s with cannot add statistics")
	}
	...
	err = st.Init()
	if err != nil {
		return nil, fmt.Errorf("Cannot add statistics %s with %s", err)
	}
...
/* This is the statistics for a particular port .*/

func (p *Port) InitStatistics() error {
	if p.IsStatisticsConfigured() {
		_, err := p.SyncStatistics()
		if err != nil {
			return fmt.Errorf("Cannot init statistics with: %s", err)
		}

		return nil
	}

	_, err := p.AddStatistics()
	if err != nil {
		return fmt.Errorf("Cannot init statistics with: %s", err)
	}
...
func (p *Port) GetRxRate() uint64 {
	return p.Statistics.GetRxRate()
}

func (p *Port) GetTxRate() uint64 {
	return p.Statistics.GetTxRate()
}

func (p *Port) GetRxTotal() uint64 {
	return p.Statistics.GetRxTotal()
}

func (p *Port) GetTxTotal() uint64 {
	return p.Statistics.GetTxTotal()
}
...
func (p *Port) AddLegacyHost(vid, ip, masklen, mac string) error {
	pool, err := p.LegacyLinkGetDefaultAddressPool()
	if err != nil {
		return fmt.Errorf("Cannot Add legacy host on port %s with: %s", p.Name, err)
	}

	err = pool.SetTesterAddress(vid, ip, masklen, mac)
	if err != nil {
...
func (p *Port) AddLegacyHosts(vid, ip, masklen, mac, count string) error {
	pool, err := p.LegacyLinkGetDefaultAddressPool()
	if err != nil {
		return fmt.Errorf("Cannot Add legacy hosts on port %s with: %s", p.Name, err)
	}

	err = pool.SetTesterAddresses(vid, ip, masklen, mac, count)
	if err != nil {
		return fmt.Errorf("Cannot set tester ip address for port %s with: %s", p.Name, err)
	}
...
func (p *Port) AddLegacyHost6(vid, ip, masklen, mac string) error {
	pool, err := p.LegacyLinkGetDefaultIP6AddressPool()
	if err != nil {
		return fmt.Errorf("Cannot Add legacy host on port %s with: %s", p.Name, err)
	}

	err = pool.SetTesterAddress6(vid, ip, masklen, mac)
	if err != nil {
		return fmt.Errorf("Cannot set tester ip address for port %s with: %s", p.Name, err)
	}
...
func (p *Port) AddLegacyHosts6(vid, ip, masklen, mac, count string) error {
	pool, err := p.LegacyLinkGetDefaultIP6AddressPool()
	if err != nil {
		return fmt.Errorf("Cannot Add legacy hosts on port %s with: %s", p.Name, err)
	}

	err = pool.SetTesterAddresses6(vid, ip, masklen, mac, count)
	if err != nil {
		return fmt.Errorf("Cannot set tester ip address for port %s with: %s", p.Name, err)
	}
...
func (p *Port) LegacyLinkSet(vid, testerip, tplen, testmac, sutip string) error {
	...
	*/
	...
	pool, err := p.LegacyLinkGetDefaultAddressPool()
	if err != nil {
		return fmt.Errorf("Cannot get default address pool for port %s with: %s", p.Name, err)
	}
...
func (p *Port) LegacyLinkAddSutIPAddress(ip string) error {
	...
	pool, err := p.LegacyLinkGetDefaultAddressPool()
	if err != nil {
		return fmt.Errorf("Cannot Add legacy sut on port %s with: %s", p.Name, err)
	}

	err = pool.EnableTrafficDestination()
	if err != nil {
		return fmt.Errorf("Cannot add legacy sut on port %s with %s", p.Name, err)
	}
...
func (p *Port) GetAvailableMediaTypes() ([]string, error) {
	...
	for _, field := range fields {
		if strings.TrimSpace(field) == "AGT_MEDIA_SFP" {
			p.MediaTypes[MEDIA_SFP] = strings.TrimSpace(field)
		} else if strings.TrimSpace(field) == "AGT_MEDIA_RJ45" {
			p.MediaTypes[MEDIA_RJ45] = strings.TrimSpace(field)
		} else {
...
func (p *Port) AddOSPF6(area, rid, srid, name string) (*OSPF, error) {
	cmd := fmt.Sprintf("AgtTestTopology AddSession %s AGT_SESSION_OSPFV3", p.Handler)
	res, err := p.Invoke(cmd)
	if err != nil {
		return nil, fmt.Errorf("Cannot add ospfv3 session on port %s : %s", p.Name, err.Error())
	}
...
func (p *Port) GetOSPF6ByName(name string) (*OSPF, error) {
	if len(p.OSPFs) == 0 {
		_, err := p.GetAllOSPFs()
		if err != nil {
			if err == ErrorNoOSPFSession {
...
func (p *Port) RemoveOSPF6(o *OSPF) error {
	cmd := fmt.Sprintf("AgtTestTopology RemoveSession %s", o.Handler)
	_, err := p.Invoke(cmd)
	if err != nil {
		return fmt.Errorf("Cannot remove ospf session on port %s : %s", p.Name, err.Error())
...
//AgtTestTopology ListSessions
func (p *Port) GetAllOSPF6s() ([]*OSPF, error) {
	cmd := fmt.Sprintf("AgtTestTopology ListSessions %s", p.Handler)
	res, err := p.Invoke(cmd)
	if err != nil {
		return nil, fmt.Errorf("Cannot remove ospf session on port %s : %s", p.Name, err.Error())
	...
	p.OSPF6s = make(map[string]*OSPF, 1)
	ospfs := make([]*OSPF, 0, 1)
...
func (p *Port) DeleteAllOSPF6s() error {
	ospfs, err := p.GetAllOSPF6s()
	if err != nil {
		if err == ErrorNoOSPFSession {
			return nil
		}
...
func (p *Port) AddTraffic(scount, name string) (*Traffic, error) {
	...
	err = nt.AddStatistics()
	if err != nil {
		return nil, fmt.Errorf("Cannot Init statistics for traffic %s with %s", name, err)
	}
...
func (p *Port) AddStreams(name string, dport *Port) (*Traffic, error) {
	cmd := fmt.Sprintf("AgtProfileList AddProfile %s AGT_CONSTANT_PROFILE", p.Handler)
	res, err := p.Invoke(cmd)
	if err != nil {
		return nil, fmt.Errorf("Cannot add traffic with: %s", err)
...
func (p *Port) Sync() error {
	...
	err = p.InitStatistics()
	if err != nil {
		return fmt.Errorf("Cannot init port %s with %s", p.Name, err)
	}
...
func (p *Port) GetStatistics() (uint64, uint64, uint64, error) {
	return p.Statistics.GetStatistics()
}
