...
type RUT struct {
	...
	PromptHostname string //hostName
	SessionID      string
	SFU            string
	ActiveSFU      string
	API
...
func (d *RUT) Init() error {
	if d.Device == "V8" {
		...
		d.BasePrompt = d.Hostname + "[" + d.SFU + "/" + d.ActiveSFU + "]"
	} else {
	...
	pname, rname, err := c.GetHostname()
	if rname != "" && rname != d.Hostname {
		d.Hostname = rname
		d.PromptHostname = pname
...
func (d *RUT) UpdateConfiguration() {
	//log.Println(r.Hostname, r.Device)
	var conf configuration.Configuration
	conf.Name = d.Name
	...
	conf.PromptHostname = d.PromptHostname
...
func (d *RUT) Put(server, local string) error {
...
func (d *RUT) Get(server, remote string) error {
	s, ok := d.FileServers[server]
	if !ok {
		return fmt.Errorf("remote server %s is not exist", server)
	}
