...
func main() {
	...
	fmt.Println(sts)
	tick := time.Tick(time.Duration(time.Millisecond * 10))
	for k, _ := range sts {
		var pfx string

		if strings.HasPrefix(k, "60") ||
			strings.HasPrefix(k, "90") ||
			strings.HasPrefix(k, "7") {
			pfx = "0"
		} else if strings.HasPrefix(k, "00") ||
			strings.HasPrefix(k, "20") ||
			strings.HasPrefix(k, "3") {
			pfx = "1"
		} else {
			continue
		}

		url := fmt.Sprintf(histFmt, pfx, k)
