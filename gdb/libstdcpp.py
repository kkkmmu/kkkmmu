    ...
    def __init__(self, typename, val):
        ...
        if mgr != 0:
            ...
            if 'std::string' in mgrname:
                mgrname = re.sub("std::string(?!\w)", str(
                    gdb.lookup_type('std::string').strip_typedefs()), m.group(1))
...
class StdExpOptionalPrinter(SingleObjContainerPrinter):
    ...
    def __init__(self, typename, val):
        ...
        self.typename = re.sub(
            '^std::(experimental::|)(fundamentals_v\d::|)(.*)', r'std::\1\3<%s>' % valtype, typename, 1)
...
class StdVariantPrinter(SingleObjContainerPrinter):
    ...
    def __init__(self, typename, val):
        ...
        self.typename = strip_versioned_namespace(typename)
        self.typename = "%s<%s>" % (self.typename, ', '.join(
            [self._recognize(alt) for alt in alternatives]))
